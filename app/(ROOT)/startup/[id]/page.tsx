...
const Startup = async ({params}: {params: Promise<{id: string}>}) => {
  ...
  return (
    <>
      <section className="pink_container !ming-h-[230px]">
        <p className="tag">{formatDate(post?._createdAt)}</p>
        <h1 className="heading">{post.title}</h1>
        <p className="sub-heading !max-w-5xl">{post.description}</p>
      </section>
      <section className="section_container">
        <img src={post.image} alt="photo" className="w-full h-auto rounded-xl" />
        <div className="space-y-5 mt-10 max-w-4xl mx-auto">
          ...
          {parsedContent ? <article className="prose max-w-4xl font-work-sans break-all" dangerouslySetInnerHTML={{__html: parsedContent}} /> : <p>No results</p>}
        </div>
